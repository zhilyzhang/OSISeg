...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        if self.gamma_1 is None:
            x = x + self.drop_path(y) + self.Space_Adapter(x)
            x = x + self.drop_path(self.mlp(self.norm2(x))) + self.MLP_Adapter(x)

            # x = x + self.drop_path(y)
            # x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * y) + self.Space_Adapter(x)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[512], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.feature_size = img_size[0] // patch_size
        ...
        self.num_block = self.get_num_layers()
    ...
    def use_pre_weights(self, pre_weight_path):
        pre_weights = torch.load(pre_weight_path, map_location='cpu')
        pre_weights = pre_weights['model']
        # print([k for k in pre_weights.keys()])
        encoder_state_dict = {}
        for k, v in pre_weights.items():
            if 'encoder_' in k:
                new_k = k[len('encoder_'):]
                encoder_state_dict[new_k] = v

            if k in ['cls_token', 'patch_embed.proj.weight', 'patch_embed.proj.bias']:
                encoder_state_dict[k] = v

            if k.startswith("encoder."):
                # 移除前缀以对齐我们在 MAEEncoder 里定义的名字
                # 例如 "encoder.patch_embed.proj.weight" -> "patch_embed.proj.weight"
                new_k = 'blocks.' + k[len("encoder."):]
                encoder_state_dict[new_k] = v

            if 'pos_embed' in k and 'encoder_' in k:
                new_k = k[len('encoder_'):]
                # if 'encoder_' in k:
                #     new_k = k[len('encoder_'):]
                # else:
                #     new_k = k
                _, s_len, channels = pre_weights[k].shape
                ori_size = int((s_len - 1) ** 0.5)
                if self.feature_size != ori_size:
                    pos_tokens = pre_weights[k][:, 1:, :].reshape((1, ori_size, ori_size, -1)).permute(0, 3, 1, 2)
                    pos_tokens = F.interpolate(pos_tokens, size=(self.feature_size, self.feature_size), mode='bicubic',
                                               align_corners=False)
                    pos_tokens = pos_tokens.permute(0, 2, 3, 1).reshape((1, -1, channels))
                    new_pos_tokens = torch.cat([pre_weights[k][:, :1, :], pos_tokens], dim=1)
                    encoder_state_dict[new_k] = new_pos_tokens

        load_info = self.load_state_dict(encoder_state_dict, strict=False)
        print(load_info)
    ...
    def forward(self, x, return_all_tokens=None, mask=None):
        ...
        #     return x
        feature = x[:, 1:, :]
        b, _, channels = feature.shape
        feature = feature.reshape((b, hw_shape, hw_shape, channels)).permute((0, 3, 1, 2))
        x = self.neck(feature)
        return x
...
def load_pretrained_mae_encoder(model, checkpoint_path):
    """
...
if __name__ == "__main__":
    ...
    net = vit_base()
    net.use_pre_weights(pretrained_path)
