...
from .common import LayerNorm2d, MLPBlock, SpatialAdapter
...
class _LoRA_qkv(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.qkv = qkv
        self.linear_a_q_LoRA = linear_a_q
        self.linear_b_q_LoRA = linear_b_q
        self.linear_a_v_LoRA = linear_a_v
        self.linear_b_v_LoRA = linear_b_v
        self.dim = qkv.in_features
        self.w_identity = torch.eye(qkv.in_features)

    def forward(self, x):
        qkv = self.qkv(x)  # B,N,N,3*org_C
        new_q = self.linear_b_q_LoRA(self.linear_a_q_LoRA(x))
        new_v = self.linear_b_v_LoRA(self.linear_a_v_LoRA(x))
        qkv[:, :, :, : self.dim] += new_q
        qkv[:, :, :, -self.dim:] += new_v
        return qkv
...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pos_embed is not None:
            ...
            if self.pos_embed.shape[1:3] == hw_shape:
                x = x + self.pos_embed
            else:
                pos_embed = self.pos_embed.permute((0, 3, 1, 2))
                pos_embed = F.interpolate(pos_embed, hw_shape, mode='bilinear')
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.w_Bs = []
        self.MLP_Adapter = SpatialAdapter(dim, skip_connect=False)  # MLP-adapter, no skip connection
        w_qkv_linear = self.attn.qkv
        self.w_a_linear_q_LoRA = nn.Linear(dim, self.r, bias=False)
        self.w_b_linear_q_LoRA = nn.Linear(self.r, dim, bias=False)
        self.w_a_linear_v_LoRA = nn.Linear(dim, self.r, bias=False)
        self.w_b_linear_v_LoRA = nn.Linear(self.r, dim, bias=False)
        self.attn.qkv = _LoRA_qkv(
            w_qkv_linear,
            self.w_a_linear_q_LoRA,
            self.w_b_linear_q_LoRA,
            self.w_a_linear_v_LoRA,
            self.w_b_linear_v_LoRA,
            )
