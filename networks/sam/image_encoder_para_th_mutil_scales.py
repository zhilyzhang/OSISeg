...
from .common import LayerNorm2d, MLPBlock, Adapter, SpatialAdapterV2
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.Depth_Adapter = SpatialAdapterV2(dim, skip_connect=False)  # with skip connection
        self.scale = scale
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = shortcut + x
        xn = self.norm2(x)
        x = x + self.mlp(xn) + self.scale * self.MLP_Adapter(xn) + self.Depth_Adapter(x)
        return x
