...
from dataset.utils import polys2mask, random_click, scale_bbox_xywh_to_xyxy,\
    sample_points_from_binary_mask, cal_center_point_for_binary_mask
...
class InterSegBasedPoints(object):
    '''
    points, bbox, 一次交互情况，
    '''
    def __init__(self,
        ...
        self.choice_point_type = 'center_point'
    ...
    def pred_gt_mask2prompt_point(self, pred, gt_mask):
        ...
        if np.sum(mistaken_mask) > 50:  # 负样本点
            ...
            if len(point_coords) == 0:
                # point_coord = random_click(mistaken_mask, value=1)
                if self.choice_point_type == 'center_point':
                    pt = cal_center_point_for_binary_mask(mistaken_mask)
                else:
                    pt = random_click(np.array(mistaken_mask), value=1)
                point_coords = [pt]
    ...
    def make_batch_sample(self, batch_image_path, batch_gt_json_path):
        # prompt_type = np.random.randint(1, self.type_num)
        # prompt_type = np.random.choice([1, 3])
        prompt_type = 1
        ''' 提示类型：
                ① 只有一个正样本点；
                ② 只有一个bbox
...
class InterSegBasedNPoints(object):
    '''
    points, bbox, 一次交互情况，
    '''
    def __init__(self,
        ...
        if debug:
            gt_json_paths = copy.deepcopy(self.gt_json_paths)
            self.gt_json_paths = [s[:100] for s in gt_json_paths if len(s) > 6]
            del gt_json_paths
        self.model = model
        self.device = device
        self.box_scales = box_scales
    ...
    def pred_gt_mask2prompt_point(self, pred, gt_mask):
        ...
        mistaken_mask = np.maximum(pred_mask - gt_mask, 0)
        # mode 1:
        if np.sum(mistaken_mask) > np.sum(missed_mask):  # 负样本点
            point_label = 0
            point_coords = sample_points_from_binary_mask(mistaken_mask, min_area=20, border_width=2)
            if len(point_coords) == 0:
                if self.choice_point_type == 'center_point':
    ...
    def interactive_sample(self, ann, shape, mask):
        ...
        with torch.no_grad():
            out = self.model.test_mask_forward(image_embeddings=self.image_embeddings,
                                               ...
                                               img_pad_shape=shape)
        points = self.pred_gt_mask2prompt_point(pred=out, gt_mask=mask)
        iter_num = np.random.randint(1, 4)  # 4
        # iter_num = 3
        while iter_num > 1:
            out = self.point_mask_prompt2mask(pred=out, points=points)
            points = self.pred_gt_mask2prompt_point(pred=out, gt_mask=mask)
    ...
    def make_batch_sample(self, batch_image_path, batch_gt_json_path):
        # prompt_type = np.random.randint(1, self.type_num)
        # prompt_type = np.random.choice([1, 3, 3, 3])
        prompt_type = 3
        ''' 提示类型：
                ① 只有一个正样本点；
                ② 只有一个bbox
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_file", type=str, default='cfg_evlab_buildingV2')
    ...
    state_dict = torch.load('/home/zzl/experiments/InterSegAdapter/sam_para_data_update_std/EVLabBuilding_4-time_continuous_center_point_prompt_epo_100_bs_8/weight_epoch_38_IoU_0.8884.pth', map_location='cpu')
    net.load_state_dict(state_dict)
    iter_seg_data = InterSegBasedNPoints(image_paths=train_image_paths,
