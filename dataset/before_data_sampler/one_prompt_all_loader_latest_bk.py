...
class InterSegOnePrompt(object):
    ...
    def drag_hole_to_mask_and_processing(self, binary_image):
        # 获取所有非零点的坐标
        non_zero_points = np.argwhere(binary_image > 0)
        ...
        # 挖掉区域
        # binary_image[min_row:max_row, min_col:max_col] = 0
        clip_mask = binary_image[min_row:max_row, min_col:max_col]
        clip_mask[(max_row-min_row)//2, (max_col-min_col)//2] = np.random.randint(0, 2)
        clip_mask = random_dilate_or_erode(clip_mask)
        binary_image[min_row:max_row, min_col:max_col] = clip_mask
        return binary_image
    ...
    def drag_hole_random_dilate_or_erode_threshold(self, pred_mask, gt_mask):
        # 尝试直到 IoU 低于 0.6
        iou = 1.0
        attempts = 0
        while iou >= 0.6 and attempts < 5:
            # pred_mask = random_dilate_or_erode(pred_mask)
            pred_mask = self.drag_hole_to_mask_and_processing(pred_mask)
            iou = calculate_iou(gt_mask, pred_mask)
            attempts += 1
        return pred_mask
    ...
    def make_batch_sample(self, batch_image_path, batch_gt_json_path):
        ...
        for i in range(self.batch_size):
            ...
            if prompt_type == 1:
                ...
                pred_mask = self.drag_hole_random_dilate_or_erode_threshold(pred_mask, gt_mask)
