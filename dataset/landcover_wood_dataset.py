...
class WoodLandDataset:
    def __init__(self, image_dir, gt_mask_dir,
        ...
        self.data_pts = []
        if self.mode == 'test':
            self.data_pts = glob(osp.join('/home/zzl/datasets/landcover.ai.dataset/test_data',
                                          '*.pt'))
        elif self.mode == 'test_vis':
            self.data_pts = glob(osp.join('/home/zzl/datasets/landcover.ai.dataset/test_data_vis_ref/test_vis_data',
                                          '*.pt'))
        else:
            self.load_data()
        ...
        # 数据信息
        if debug:
            if self.mode == 'test' or self.mode == 'test_vis':
                self.data_pts = self.data_pts[:20]
            else:
...
if __name__ == '__main__':
    # Example usage
    image_dir = '/home/zzl/datasets/landcover.ai.dataset/test/img'
    ann_path = '/home/zzl/datasets/landcover.ai.dataset/test/label.woodland'

    # Initialize dataset
    iter_seg_data = WoodLandDataset(image_dir=image_dir, gt_mask_dir=ann_path,
                             debug=False,
                             mode='test',
                             model=None,
                             device=None,
    ...
    save_test_sample_dir = '/home/zzl/datasets/landcover.ai.dataset/test_data'
