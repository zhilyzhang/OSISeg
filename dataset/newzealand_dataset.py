...
class NewZealandDataset:
    def __init__(self, image_dir, gt_mask_dir,
        ...
        self.data_pts = []
        if self.mode == 'test':
            self.data_pts = glob(osp.join('/home/zzl/datasets/building_data/NewZealand-building-dataset/test_data',
                                          '*.pt'))
        # 数据信息
        if debug:
            if self.mode == 'test':
    ...
    def make_batch_sample(self, batch_filenames):
        ...
        for i in range(self.batch_size):
            ...
            assert num_features > 0, '没有连通区域'

            # 随机选择一个连通区域（标签从1到num_features）
            list_num_label = list(range(1, num_features + 1))
...
if __name__ == '__main__':
    # Example usage
    image_dir = '/home/zzl/datasets/building_data/NewZealand-building-dataset/test/image'
    ann_path = '/home/zzl/datasets/building_data/NewZealand-building-dataset/test/binary_map'

    # Initialize dataset
    iter_seg_data = NewZealandDataset(image_dir=image_dir, gt_mask_dir=ann_path,
                             debug=False,
                             mode='val',
                             model=None,
                             device=None,
    ...
    save_test_sample_dir = '/home/zzl/datasets/building_data/NewZealand-building-dataset/test_data'
