...
def _build_sam_ori(
    ...
    # elif args.adapter == 'sam_mix':
    #     from models.sam.modeling.image_encoder_mix import ImageEncoderViT
    # elif args.adapter == 'sam_series':
    #     from models.sam.modeling.image_encoder_series import ImageEncoderViT
    # elif args.adapter == 'sam_para':
    #     from models.sam.modeling.image_encoder_para import ImageEncoderViT
    # elif args.adapter == 'sam_lora':
    #     from models.sam.modeling.image_encoder_lora import ImageEncoderViT
    from models.sam.modeling.image_encoder import ImageEncoderViT
    sam = Sam(
        args,
    ...
    if checkpoint is not None:
        ...
        if len(unexpected_keys) == 0:
            print('load sam pre-weight successfully!')
        # 打印信息
        if missing_keys:
...
def _build_sam(
    ...
    assert os.path.exists(checkpoint), f'{checkpoint} must exist!'
    from .weight_transform import load_weight_by_splited_models
    load_weight_by_splited_models(weight_path=checkpoint, image_encoder=image_encoder,
                                  mask_decoder=mask_decoder, prompt_encoder=prompt_encoder)
